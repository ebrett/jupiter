...
RSpec.describe Request, type: :model do
  describe 'validations' do
    ...
    it 'validates amount_requested is positive' do
      request = Request.new(amount_requested: -10)
      request.valid?
    ...
    it 'validates exchange_rate is positive' do
      request = Request.new(exchange_rate: -1)
      request.valid?
  ...
  describe 'enums' do
    ...
    it 'defines correct request_type values' do
      expect(Request.request_types).to eq({
        'reimbursement' => 'R',
        'vendor' => 'V',
        'inkind' => 'I'
      })
  ...
  describe 'scopes' do
    let!(:inkind_request) { Request.create!(request_type: 'inkind', amount_requested: 100, form_data: { test: 'data' }) }
    let!(:reimbursement_request) { Request.create!(request_type: 'reimbursement', amount_requested: 200, form_data: { test: 'data' }) }

    describe '.by_type' do
      it 'filters by request type' do
        results = Request.by_type('inkind')
